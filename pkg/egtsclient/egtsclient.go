...
func Start(addr string, numOIDs int, time int) {
	...
	defer conn.Close()
	log.Printf("EGTS client was started. Server address: %v", addr)
	if err != nil {
		log.Printf("got error: %v", err)
	}
	go sendData(conn, time, uint32(numOIDs))
	receiveReply(conn)
}

func sendData(conn net.Conn, sendPeriod int, numOIDs uint32) {
	var packNum uint16
	var recNum uint16
	var err error
...
func sendNewMessage(conn net.Conn, numOIDs uint32, packNum uint16, recNum uint16) (uint16, uint16, error) {
	...
	packetNav, err := packetData.Form()
	if err != nil {
		return packNum, recNum, err
	}
	parsedPacket := new(egts.Packet)
